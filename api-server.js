 ...
 * GET  /api/series/:slug/:seasonParam/:epParam
 * GET  /api/movies/:slug
 */
 ...
 */
app.get('/api/series/:slug/:seasonParam/:epParam', async (req, res) => {
  const slug = (req.params.slug || '').trim().replace(/\/+$/, '');
  const seasonNum = parseSeasonParam(req.params.seasonParam);
  const epParam = (req.params.epParam || '').trim();

  if (!slug) return res.status(400).json({ error: 'slug requerido' });
  if (!seasonNum || seasonNum < 1) {
    return res.status(400).json({ error: 'temporada inválida' });
  }
  if (!epParam) return res.status(400).json({ error: 'episodio requerido' });

  log(`API episodio: ${slug} T${seasonNum} ep=${epParam}`);
  try {
    const catalog = await loadCatalog('series');
    const series = findInCatalog(catalog, slug) || buildSeriesFromSlug(slug);
    ensureDir(USER_DATA_DIR);

    const details = await scrapeOneEpisode(series, seasonNum, epParam, {
      resolveServers: req.query.resolve !== '0',
      userDataDir: USER_DATA_DIR
    });
    res.json(details);
  } catch (err) {
    log(`ERROR /api/series/${slug}/temp${seasonNum}/${epParam}: ${err.message}`);
    res.status(500).json({
      slug,
      temporada: seasonNum,
      episodio: epParam,
